...
# 用途：在 Kaldi 合并数据上微调 NVIDIA NEST XLarge + DiariZen Conformer 头。
# 默认使用 HuggingFace/NeMo 模型 nvidia/ssl_en_nest_xlarge_v1.0 代替 WavLM。
# 示例：
#   bash run_finetune_kaldi_merged_nest_xlarge_1219_all.sh
#   CUDA_VISIBLE_DEVICES=0,1 NUM_GPUS=2 BATCH_SIZE=64 bash run_finetune_kaldi_merged_nest_xlarge_1219_all.sh
#   SKIP_TRAIN=1 bash run_finetune_kaldi_merged_nest_xlarge_1219_all.sh  # 只生成配置
...
TMUX_SESSION_NAME="diarizen_nest_xlarge_ft"
LOG_FILE="$SCRIPT_DIR/run_finetune_kaldi_merged_nest_xlarge_1219_all.log"
...
EXP_NAME="${EXP_NAME:-kaldi_merged_1219_all_ft_nest_xlarge_2gpu}"
...
# 默认面向本机 2 x A800 80GB；每卡 batch=64 已实测可启动。
BATCH_SIZE="${BATCH_SIZE:-64}"
VAL_BATCH_SIZE="${VAL_BATCH_SIZE:-64}"
MAX_EPOCHS="${MAX_EPOCHS:-30}"
LR_NEST="${LR_NEST:-5e-6}"
...
# 0 表示从 NeMo 模型自动推断。xlarge 通常为 24 层、1024 维。
...
# 只用于从本 recipe 训练出的 NEST-DiariZen checkpoint 继续初始化；
# 首次使用 HF/NeMo NEST 权重时保持 FINETUNE=false。
...
if [[ "$FINETUNE" == "true" ]]; then
    if [[ -z "$CKPT_DIR" ]]; then
        echo "FINETUNE=true requires CKPT_DIR to point to a DiariZen NEST checkpoint." >&2
        exit 1
    fi
    if [[ ! -e "$CKPT_DIR" ]]; then
...
echo "[4/4] Launching NEST fine-tuning with accelerate on $NUM_GPUS GPU(s)..."
