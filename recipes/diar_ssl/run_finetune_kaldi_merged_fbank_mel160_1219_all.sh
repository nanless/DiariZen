...
# 用途：从已有 checkpoint 继续训练 fbank-conformer 模型（调整学习率）
# 前提：已有训练好的 checkpoint 目录
# 示例：
#   bash run_finetune_kaldi_merged_fbank.sh                         # 默认配置
#   CKPT_DIR=/path/to/ckpt LR=5e-5 bash run_finetune_kaldi_merged_fbank.sh
...
TMUX_SESSION_NAME="diarizen_fbank_mel160_finetune"
LOG_FILE="$SCRIPT_DIR/run_finetune_kaldi_merged_fbank_mel160_1219_all.log"
...
# 原实验配置（保持与原训练一致）
EXP_NAME="${EXP_NAME:-kaldi_merged_1205_1207_fbank_conformer_mel160_1219_all}"
DATA_OUT="$RECIPE_DIR/data/$EXP_NAME"
# 使用 epoch_0037（最好的 checkpoint），代码会识别 checkpoints/epoch_ 模式直接加载
BASE_CKPT_DIR="${BASE_CKPT_DIR:-$RECIPE_DIR/exp/$EXP_NAME/checkpoints/epoch_0023}"

# 新的 finetune 实验配置
FINETUNE_EXP_NAME="${FINETUNE_EXP_NAME:-${EXP_NAME}_finetune}"
SEED="${SEED:-3407}"
NUM_GPUS="${NUM_GPUS:-$NUM_GPUS}"
PORT="${PORT:-11348}"  # 使用不同端口避免冲突
...
if [[ ! -f "$TRAIN_DIR/wav.scp" || ! -f "$DEV_DIR/wav.scp" ]]; then
    echo "Error: Data not found. Please run run_train_kaldi_merged_fbank.sh first to prepare data." >&2
    exit 1
fi
