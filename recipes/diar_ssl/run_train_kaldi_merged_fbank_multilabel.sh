...
# 用途：在 Kaldi 合并数据上从头训练 fbank-conformer 模型（使用原生 multilabel，不使用 powerset）。
# 前提：已准备好 Kaldi 三件套（wav.scp / rttm / reco2dur）。
# 示例：
#   bash run_train_kaldi_merged_fbank_multilabel.sh                             # 默认 2 卡，默认数据路径
#   DATA_SRC=/data/kaldi EXP_NAME=my_exp NUM_GPUS=2 bash run_train_kaldi_merged_fbank_multilabel.sh
#   FORCE_REBUILD_DATA=1 VAL_RATIO=0.2 bash run_train_kaldi_merged_fbank_multilabel.sh
#
# 注意：脚本会自动在 tmux session 中运行，确保断开连接后训练继续。
#       使用 tmux attach -t diarizen_fbank_multilabel_train 查看训练状态。
...
TMUX_SESSION_NAME="diarizen_fbank_multilabel_train"
LOG_FILE="$SCRIPT_DIR/run_train_kaldi_merged_fbank_multilabel.log"
...
# 注意：此脚本使用原生 multilabel（不使用 powerset），损失函数为 binary_cross_entropy
...
SEED="${SEED:-3407}"
NUM_GPUS="${NUM_GPUS:-$NUM_GPUS}"      # default 2 from env defaults
PORT="${PORT:-11348}"                   # different port to avoid conflict
...
LR="${LR:-3e-4}"                         # larger lr for training from scratch
...
# 注意：不设置 max_speakers_per_frame，以使用原生 multilabel 模式
cat > "$CONF_OUT" <<EOF
[meta]
save_dir = "exp"
seed = $SEED
...
# - fbank conformer 从头训练，使用单优化器，原生 multilabel（不使用 powerset）
...
# - max_speakers_per_frame 在 multilabel 模式下不使用，但需要提供默认值
# - use_powerset = false：使用 multilabel 模式
#   - 模型输出直接是 (B, T, num_speakers) 的 sigmoid 概率
#   - 损失函数使用 binary_cross_entropy 而不是 nll_loss
...
# - 输出同时显示在终端和日志文件中
# - 使用 run_single_opt.py 进行单优化器训练（从头训练，原生 multilabel）
accelerate launch \
