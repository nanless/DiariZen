...
# Purpose: continue fine-tuning from the latest trained NEST xlarge checkpoint.
# Defaults:
# - source experiment: kaldi_merged_1219_all_ft_nest_xlarge_2gpu
# - auto-pick latest checkpoint under exp/<SRC_EXP_NAME>/checkpoints/epoch_XXXX
# - new learning rates: LR_NEST=1e-6, LR_HEAD=1e-4
#
# Examples:
#   bash run_finetune_kaldi_merged_nest_xlarge_1219_all_continue.sh
#   SRC_EXP_NAME=kaldi_merged_1219_all_ft_nest_xlarge_2gpu EXP_NAME=my_next_stage \
#     bash run_finetune_kaldi_merged_nest_xlarge_1219_all_continue.sh
#   CKPT_DIR=/path/to/checkpoints/epoch_0012 NUM_GPUS=1 \
#     bash run_finetune_kaldi_merged_nest_xlarge_1219_all_continue.sh
...
TMUX_SESSION_NAME="diarizen_nest_xlarge_ft_continue"
LOG_FILE="$SCRIPT_DIR/run_finetune_kaldi_merged_nest_xlarge_1219_all_continue.log"
...
SRC_EXP_NAME="${SRC_EXP_NAME:-kaldi_merged_1219_all_ft_nest_xlarge_2gpu}"
EXP_NAME="${EXP_NAME:-${SRC_EXP_NAME}_continue_lr1e-6_1e-4}"
...
if [[ -z "$CKPT_DIR" ]]; then
    ...
    mapfile -t checkpoints < <(find "$SRC_CHECKPOINTS_DIR" -maxdepth 1 -type d -name "epoch_[0-9][0-9][0-9][0-9]" | sort)
    if [[ ${#checkpoints[@]} -eq 0 ]]; then
        echo "No checkpoints found in $SRC_CHECKPOINTS_DIR" >&2
        exit 1
    fi
